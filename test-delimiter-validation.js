...
tests.forEach((test, index) => {
	...
	console.log(
		`  v1 result: ${resultV1} (expected: ${test.expectedV1}) - ${passV1 ? '✅ PASS' : '❌ FAIL'}`
	);
	console.log(
		`  v2 result: ${resultV2} (expected: ${test.expectedV2}) - ${passV2 ? '✅ PASS' : '❌ FAIL'}`
	);
...
console.log('='.repeat(80));
console.log(
	`v1 (old): ${passedV1}/${totalTests} tests passed (${Math.round((passedV1 / totalTests) * 100)}%)`
);
console.log(
	`v2 (new): ${passedV2}/${totalTests} tests passed (${Math.round((passedV2 / totalTests) * 100)}%)`
);
console.log();
